        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              ...
              {homework.sectionId?.name ? ` · ${homework.sectionId.name}` : ""}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              return (
                ...
                >
                  ...
                  <textarea
                    ...
                    placeholder="Remark (visible to parent & student)"
...
export default function HomeworkSchool() {
  ...
  const fetchList = async () => {
    try {
      const res = await axios.get(`${API}/homework/school`, {
        withCredentials: true,
      });
      setList(res.data || []);
    } catch (err) {
      toast.error(err.response?.data?.error || "Failed to load homework");
    }
  };

  useEffect(() => {
    const init = async () => {
      await Promise.all([fetchClasses(), fetchList()]);
      setLoading(false);
    };
    init();
  }, []);
  ...
  const handleSectionChange = (sectionId) => {
    ...
    const seen = new Set();
    const filtered = (sec?.subjectTeachers || []).filter((st) => {
      const key = st.subjectId?._id?.toString();
  ...
  const openReview = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Failed to load student statuses");
    } finally {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 text-[rgb(var(--text))]">
      ...
      <h1 className="text-lg font-semibold mb-4">Homework management</h1>
      ...
      <div className="space-y-3">
        <p className="text-sm font-medium">School homework</p>
        {loading ? (
                ...
                <div className="flex justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      ...
                      {hw.classId?.name}
                      {hw.sectionId?.name ? ` · ${hw.sectionId.name}` : ""} ·{" "}
                      {hw.teacherId?.fullName ||
                        hw.teacherId?.name ||
                        "Staff/Admin"} · Due {fmtDate(hw.dueDate)}
                    </p>
