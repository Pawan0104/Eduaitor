...
import { FaArrowLeft, FaSearch, FaTimes, FaChevronRight, FaClipboardList, FaUserCheck } from "react-icons/fa";
import UserAvatar from "../components/UserAvatar";

const ClassAttendanceReportTeacher = lazy(() => import("./ClassAttendanceReportTeacher"));
...
/* ══════════════════════════════════════════════════════════════
   SUBJECT-WISE REPORT (inner component, original logic preserved)
══════════════════════════════════════════════════════════════ */
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Sel label="Section" value={selSection} onChange={e => setSelSection(e.target.value)} disabled={!selClass}>
            ...
            {availableSections.map(d => (
              <option key={d._id} value={d.sectionId?._id} className="bg-[rgb(var(--surface))]">{d.sectionId?.name}</option>
            ))}
          </Sel>
          ...
          <div className="col-span-2">
            ...
            <div className="relative">
              ...
              />
              {search && (
                <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[rgb(var(--text))] opacity-40 hover:opacity-80">
                  <FaTimes size={11} />
                </button>
              )}
            </div>
        ...
        <>
          ...
          <div className="bg-[rgb(var(--surface))] rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              ...
              <span className="text-xs bg-[rgb(var(--bg))] border border-slate-200 text-[rgb(var(--text))] font-medium px-2.5 py-1 rounded-full">
            ...
            <div className="px-4">
              {filteredData.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-xs text-[rgb(var(--text))] mt-1">Try a different filter</p>
                </div>
              ) : reportType === "daily" ? (
