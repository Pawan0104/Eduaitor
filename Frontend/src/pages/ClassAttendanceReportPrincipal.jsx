...
/* ══════════════════════════════════════════════════════════════
   MAIN
══════════════════════════════════════════════════════════════ */
export default function ClassAttendanceReportPrincipal() {
  ...
  const goToStudent = (record) => {
    ...
    navigate(`/school/attendance/student/${studentId}`, {
      state: { from: "class", month, year, selDate },
    });
  };

  const clearAll = () => {
    setSelClass(""); setSelSection(""); setSelDate("");
    setMonth(""); setYear(""); setSearch("");
  ...
  const classTeacherName    = availableSections.find(d => d.sectionId?._id === selSection)?.teacherId
    ? `${availableSections.find(d => d.sectionId?._id === selSection)?.teacherId?.firstName ?? ""} ${availableSections.find(d => d.sectionId?._id === selSection)?.teacherId?.lastName ?? ""}`.trim()
    : null;
    ...
    <div className="min-h-screen bg-[rgb(var(--bg))]">
      ...
      <div className="max-w-2xl mx-auto px-3 py-5 sm:px-6 sm:py-8">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm text-[rgb(var(--text))] mt-0.5">School-wide class attendance overview</p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Sel label="Section" value={selSection} onChange={e => setSelSection(e.target.value)} disabled={!selClass}>
                ...
                {availableSections.map(d => (
                  <option key={d._id} value={d.sectionId?._id} className="bg-[rgb(var(--surface))]">
                    {d.sectionId?.name}
                    {d.teacherId?.firstName ? ` (${d.teacherId.firstName})` : ""}
                  </option>
                ))}
              </Sel>

              {/* Class teacher info pill */}
              {classTeacherName && (
                <div className="col-span-2">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[rgb(var(--bg))] border border-slate-200">
                    <span className="text-xs text-[rgb(var(--text))]">Class Teacher:</span>
                    <span className="text-xs font-semibold text-[rgb(var(--text))]">{classTeacherName}</span>
                  </div>
                </div>
              )}
              ...
              <div className="col-span-2">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Name or roll number…" value={search}
              ...
              <div className="col-span-2 flex gap-2 mt-1">
                ...
                <button onClick={clearAll}
                  className="border border-slate-200 text-sm font-medium py-2.5 px-4
                    rounded-xl bg-[rgb(var(--surface))] text-[rgb(var(--text))]">
                  Clear
                </button>
              </div>
            </div>
            ...
            <div className="bg-[rgb(var(--surface))] rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-start justify-between px-4 py-3 border-b border-slate-100">
                <div>
                  ...
                  {modeLabel && <p className="text-xs text-[rgb(var(--text))] font-medium mt-0.5">{modeLabel}</p>}
              ...
              <div className="px-4">
                {reportData.length === 0 ? (
                  <div className="py-12 text-center">
                    ...
                    <p className="text-xs text-[rgb(var(--text))] mt-1">Try a different filter or date</p>
                  </div>
                ) : reportType === "daily" ? (
                  reportData.map((r,i) => <DailyRow key={r._id ?? i} record={r} onClick={() => goToStudent(r)} />)
                ) : (
                  reportData.map((r,i) => <MonthlyRow key={r._id ?? i} record={r} onClick={() => goToStudent(r)} />)
                )}
              </div>
            </div>
          </>
        ...
        {!hasFetched && !metaLoading && (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[rgb(var(--text))]">
              Select class, section then tap <strong>View Report</strong>
            </p>
          </div>
        )}
      </div>
